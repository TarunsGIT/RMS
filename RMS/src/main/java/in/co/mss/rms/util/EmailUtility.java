...
public class EmailUtility {
     ...
     */
	 ...
	 private static Properties props =new Properties();//util//SUB CLASS of HASH TABLE(HISTORICAL CLASS)(METHOD PUT();)
	 //(KEY AND VALUE ARE IN FORM OF STRING)
	    ...
	    public static void sendMail(EmailMessage emailMessageDTO) throws ApplicationException{
	    	try{
	         ...
	         // Set BCC addresses
	            ...
	            InternetAddress[] addressTo = new InternetAddress[emailIds.length];//ADDRESS To
	            
	            for (int i = 0; i < emailIds.length; i++) {
	                addressTo[i] = new InternetAddress(emailIds[i]);
	            }
	            InternetAddress[] addressCc = new InternetAddress[emailIdsCc.length];//ADDRESS CC

	            for (int i = 0; i < emailIdsCc.length; i++) {
	                addressCc[i] = new InternetAddress(emailIdsCc[i]);
	            }
	            InternetAddress[] addressBcc = new InternetAddress[emailIdsBcc.length];//ADDRESS BCC
