...
import in.co.mss.rms.util.EmailBuilder;
import in.co.mss.rms.util.EmailMessage;
import in.co.mss.rms.util.EmailUtility;
...
public class UserModel {
	...
	public UserBean findByLogin(String login) throws ApplicationException {
		...
		try {
			System.out.println("in findByPK try");
			conn = JDBCDataSource.getConnection();
	...
	public long add(UserBean bean) throws ApplicationException,
		...
		UserBean existbean = findByLogin(bean.getLogin());// FIND USER BY LOGIN

		/************************** PREVENT DUPLICATION OF LOGIN_ID ************************/
		...
		try {
			...
			conn = JDBCDataSource.getConnection();
			pk = nextPK();// AUTO INCREMENT VARIABLE
			// Get auto-generated next primary key
			System.out.println(pk + " in ModelJDBC");
			conn.setAutoCommit(false); // Begin transaction//TRANSACTION
										// HANDLING
			PreparedStatement pstmt = conn
					.prepareStatement("INSERT INTO ST_USER VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			pstmt.setInt(1, pk);
			pstmt.setString(2, DataUtility.getCapital(bean.getFirstName()));
			pstmt.setString(3, DataUtility.getCapital(bean.getLastName()));
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"select * from `ST_USER` order by `ID` desc");
		...
		Connection conn = null; // conn Object of
	...
	public List search(UserBean bean, int pageNo, int pageSize)
		 ...
		 * StringBuffer("SELECT * FROM st_user order by id desc WHERE 1=1");
		 */
	...
	/******************** SEND PASSWORD OF USER TO HIS EMAIL **************************/
	...
	public List getRoles(UserBean bean) throws ApplicationException {
		...
		System.out.println("CONTROL IN GET ROLE");
		StringBuffer sql = new StringBuffer(
		...
		try {
			...
			System.out.println(sql);
			rs.close();
			System.out.println("GET ROLE ENDED");
		} catch (Exception e) {
			log.error("Database Exception..", e);
	...
	public void update(UserBean bean) throws ApplicationException,
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn
					.prepareStatement("UPDATE ST_USER SET FIRST_NAME=?,LAST_NAME=?,LOGIN=?,PASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,ROLE_NAME=?,UNSUCCESSFUL_LOGIN=?,GENDER=?,LAST_LOGIN=?,USER_LOCK=?,REGISTERED_IP=?,LAST_LOGIN_IP=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?");
			pstmt.setString(1, DataUtility.getCapital(bean.getFirstName()));
			pstmt.setString(2, DataUtility.getCapital(bean.getLastName()));
	...
	/********************** RESET PASSWORD OF USER WITH(AUTO GENRATED PASSWORD) ****************/
